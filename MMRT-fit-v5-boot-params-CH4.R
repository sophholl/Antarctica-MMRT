...
# Save the result
write_csv(df_long_new, "./output/MMRTfits_withCI_CH4.txt")
...
ggsave("./output/MMRTfit_facet_CI_CH4.svg")
...
# Save
write_tsv(soil_bootstrap_summary, "./output/MMRT_Topt_numerical_Cp_CH4.tsv")
...
ggplot(df_long_new, aes(TempC, rate_nmol)) +
  ...
  labs(
    title = "Pooled CH4 MMRT fit with 95% confidence interval",
    x = expression("Temperature ("*degree*C*")"),
    y = expression(CH[4]~oxidation~rate~"("*nmol~g^{-1}~h^{-1}*")")
  ) +
...
ggsave("./output/MMRTfit_Pooled_CH4_95CI.svg", width = 11.16, height = 8.65, units = "cm")
