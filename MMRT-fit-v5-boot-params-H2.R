...
# Save the result
write_csv(df_long_new, "./output/MMRTfits_withCI_H2.txt")
...
df_long_new |>
  ...
  labs(
    y = expression(H[2]~oxidation~rate~"("*nmol~g^{-1}~h^{-1}*")"),
    x = expression("Temperature ("*~degree*C*")"),
    title = "MMRT fit for bulk-soil H2 oxidation\nwith 95% confidence intervals"
  ) +
...
ggsave("./output/MMRTfit_facet_CI_H2.svg")
...
# Save
write_tsv(soil_bootstrap_summary, "./output/MMRT_Topt_numerical_Cp_H2.tsv")
...
#  geom_point(aes(color = soil), size = 2, alpha = 0.6) +
  ...
  labs(
    title = "Pooled MMRT fit with 95% confidence interval",
    x = expression("Temperature ("*degree*C*")"),
    y = expression(H[2]~oxidation~rate~"("*nmol~g^{-1}~h^{-1}*")")
  ) +
...
ggsave("./output/MMRTfit_Pooled_H2_95CI.svg", width = 11.16, height = 8.65, units = "cm")



####################################################
##### Finding mean MMRT fit with 95% prediction interval across all 9 sites #####
####################################################
"""
# Your existing pooled fit:
fit.all <- nlsLM(log(rate_nmol) ~ MMRT(TempK, H, S, Cp),
...
params <- coef(fit.all)
vcov_mat <- vcov(fit.all)
sigma <- summary(fit.all)$sigma  # residual standard error

# Create new temperature data
tempseq <- tibble(
  TempK = seq(253.15, 353.15, by = 1),
  TempC = TempK - 273.15
)

# For each TempK, compute prediction and standard error using delta method
MMRT_pred <- function(par, T) {
  MMRT(T, par["H"], par["S"], par["Cp"])
}

get_pred_interval <- function(Tval) {
  # Jacobian of MMRT w.r.t. parameters at T
  grad <- grad(func = function(p) MMRT_pred(setNames(p, names(params)), Tval),
               x = unname(params))
  
  # Variance of prediction from delta method
  pred_var <- t(grad) %*% vcov_mat %*% grad
  se_pred <- sqrt(pred_var)
  
  # Prediction value
  fit_val <- MMRT_pred(params, Tval)
  
  # Total prediction SD includes residual error
  pred_sd_total <- sqrt(se_pred^2 + sigma^2)
  
  # Return a **named tibble**
  tibble(
    log_rate_pred  = fit_val,
    log_rate_lower = fit_val - 1.96 * pred_sd_total,
    log_rate_upper = fit_val + 1.96 * pred_sd_total
  )
}

# Apply across all temperatures
pred_results <- map_dfr(tempseq$TempK, get_pred_interval)
colnames(pred_results) <- c("log_rate_pred", "log_rate_lower", "log_rate_upper")

# Combine with tempseq and back-transform to original scale
tempseq <- tempseq %>%
...
# Plot: pooled fit with 95% confidence ribbon
# -------------------------------------------
ggplot(df_long, aes(TempK - 273.15, rate_nmol)) +  # Convert x to °C
  geom_point(aes(color = soil), size = 2, alpha = 0.7) +
  geom_ribbon(data = tempseq, aes(x = TempC, ymin = rate_lower, ymax = rate_upper),
              fill = "grey80", alpha = 0.5, inherit.aes = FALSE) +
  geom_line(data = tempseq, aes(x = TempC, y = rate_pred),
  ...
  labs(
    title = "Pooled H2 MMRT fit with 95% prediction interval",
    x = expression("Temperature ("*degree*C*")"),
    y = expression(H[2]~oxidation~rate~"("*nmol~g^{-1}~h^{-1}*")")
  ) +
  theme_minimal(base_size = 14) +
...
ggsave("./output/MMRTfit_AllTogether_H2_95predictionInterval.svg", width = 11.16, height = 8.65, units = "cm")
"""
